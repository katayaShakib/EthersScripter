...
for (let i = 0; i < 10; i++) {
  path = `m/44'/60'/0'/0/${i}`; // you can get the path if you console.log(randomWallet.mnemonic);
...
// no need to connect to a provider to be able to sign a message. However, all of that will be happining locally
...
const mainnetProvider = new ethers.providers.JsonRpcProvider(mainnetInfuraUrl); // await provider.detectNetwork();

const exampleAccountAddress = await mainnetProvider.resolveName(process.env.EXAMPLE_ACCOUNT);
